...
export default function TodoCard(props) {
    const { children, edit, handleAddEdit,
            edittedValue, setEdittedValue, todoKey,
            handleEditTodo, handleDelete } = props
        ...
        <div className='p-2 relative sm:p-3 border flex items-stretch border-white border-solid '>
            ...
            <div className='flex-1 flex'>
                {!(edit === todoKey) ? <>{children}</> : (
                    <input className='bg-inherit flex-1 text-white outline-none' value={edittedValue}
